...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex justify-between items-center">
          ...
          <a
            ...
            className={`text-xl font-black tracking-tighter transition-colors duration-300 ${scrolled ? "text-[#1a1a1a]" : "text-[#f5f3ee]"
              }`}
          >
          ...
          <div className="hidden md:flex items-center gap-10">
            {NAV_LINKS.map((link) => (
              <button
                ...
                className={`text-[11px] font-bold uppercase tracking-[0.2em] transition-all duration-300 hover:text-electric-blue ${scrolled ? "text-[#1a1a1a]" : "text-[#f5f3ee]/80"
                  }`}
              >
                {link.name}
              </button>
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 z-[100] bg-[#1a1a1a] transition-transform duration-700 ease-[cubic-bezier(0.16, 1, 0.3, 1)] ${mobileMenuOpen ? "translate-y-0" : "-translate-y-full"
